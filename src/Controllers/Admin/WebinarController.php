...
class WebinarController extends Controller
{
    ...
    {
        //$blogposts = Post::whereNotNull('id')->take(5)->get();
       // $blogcomments = Comment::whereNotNull('id')->take(5)->get();

        return view('Webinar.Admin.index', [
        ]);
    }
    ...
    public function index(Request $request)
    {
        ...
        if($user->hasRole('superadmin')) {
            ...
            $ids = Participant::where('User_id', $user->id)->pluck('Webinar_id');
            $items = Webinar::wherein('id',$ids);
        }
    ...
    switch ($sort) {
        ...
        if ($request->ajax()) {
            return view('Webinar.Admin.load', ['items' => $items])->render();  
        }




        return view('Webinar.Admin.list', [
    ...
    public function buylist(Request $request, int $id)
    {
        ...
        if ($request->ajax()) {
            return view('Webinar.Admin.buyload', ['items' => $items])->render();  
        }

        return view('Webinar.Admin.buylist', [
            'countAll'    => Webinar::whereNotNull('id')->count(),
            'countActive' => Webinar::whereNotNull('id')->where('status', 1)->count(),
            'items'    => $items,
        ]);
    ...
    public function create()
    {
        ...
        return view('Webinar.Admin.create', [
            'grades' => $grades,
            'courses' => $courses,
            'chapters' => $chapters,
            'topics' => $topics,
    ...
    public function edit(int $id)
    {
        ...
        $provinces = Province::whereNotNull('id')->get();
        $cities = City::whereNotNull('id')->get();
        return view('Webinar.Admin.edit', [
            'item' =>  $item,
    ...
    public function karname($id){
        ...
        return view('Webinar.Admin.karname', [
            'item'    => $item,
    ...
    public function store2(Request $request){
        ...
        $quiz = Webinar::create($request->all());
            ...
            return redirect()->route('Webinar.Admin.index')->with('message', 'آزمون الفزوده شد');
